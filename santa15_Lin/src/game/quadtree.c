...
static void getAllEntsWithinNode(int x, int y, int w, int h, Entity **candidates, Quadtree *root);
...
void initQuadtree(Quadtree *root)
{
	...
	if (root->depth == 0)
	{
		SDL_LogMessage(SDL_LOG_CATEGORY_APPLICATION, SDL_LOG_PRIORITY_DEBUG, "Quadtree: [numCells = %d, memory = %.2fkb]", numCells, (double)((sizeof(Quadtree) * numCells) / 1024.0));
	}
}
...
void getAllEntsWithin(int x, int y, int w, int h, Entity **candidates)
{
	...
	getAllEntsWithinNode(x, y, w, h, candidates, &stage.quadtree);
}
...
static void getAllEntsWithinNode(int x, int y, int w, int h, Entity **candidates, Quadtree *root)
{
	...
	if (root->addedTo)
	{
		if (root->node[0] != NULL)
		{
			...
			if (index != -1)
			{
				getAllEntsWithinNode(x, y, w, h, candidates, root->node[index]);
			}
			else
			{
				for (i = 0; i < 4; i++)
				{
					getAllEntsWithinNode(x, y, w, h, candidates, root->node[i]);
				}
			}
		}
